        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {services.map((service, index) => {
            ...
            return (
              <div
                key={index}
                className={`group bg-gradient-to-br from-gray-900 to-black border border-gray-800 rounded-2xl overflow-hidden hover:border-blue-500/50 transition-all duration-300 hover:shadow-2xl hover:shadow-blue-500/10 ${
                  service.title === 'Paint Correction & Protection' ? 'relative' : ''
                }`}
              >
                {service.title === 'Paint Correction & Protection' && (
                  <div className="absolute inset-0 bg-gray-900/90 backdrop-blur-sm rounded-2xl flex items-center justify-center z-10">
